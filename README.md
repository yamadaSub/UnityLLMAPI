...
- キーはプロジェクト内に残さず、環境変数またはEditor専用のEditorPrefsから取得します。

キーの設定方法
- 解決順序（ランタイム）
  1) 環境変数
  2) Unity EditorPrefs（Editorのみ）
...
- Editorメニュー（EditorPrefs）
  - Unityメニュー: `Tools > UnityLLMAPI > Configure API Keys`
  - 入力した値は以下のEditorPrefsキーに保存されます（ビルドやVCSには含まれません）
...
- ローカル開発では環境変数の利用を推奨し、補助的にEditorPrefsを使用できます（Editor限定、ビルドには含まれません）。
...
- `Runtime/Chat/AIManager.cs` は `OpenAIApiKey` / `GrokApiKey` / `GoogleApiKey` を実行時に解決します。
  - 環境変数 →（Editor時のみ）`UnityEditor.EditorPrefs` の順で解決。
  - いずれもアセットやシーンに保存されません。

サンプル
- `Samples~/Example` に簡単な使用例があります。動作には対応する環境変数またはEditorPrefsの設定が必要です。
